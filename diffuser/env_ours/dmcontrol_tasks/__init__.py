...
def get_model_and_assets():
    ...
    return resources.GetResource(
        os.path.join(_TASKS_DIR, 'reacher_four_links.xml')
    ), common.ASSETS


def get_three_link_model_and_assets():
    """Returns a tuple containing the model XML string and a dict of assets."""
    return resources.GetResource(
        os.path.join(_TASKS_DIR, 'reacher_three_links.xml')
    ), common.ASSETS
...
def three_easy(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
    ...
    physics = Physics.from_xml_string(*get_three_link_model_and_assets())
    task = CustomReacher(target_size=_BIG_TARGET, random=random)
...
def three_hard(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
    ...
    physics = Physics.from_xml_string(*get_three_link_model_and_assets())
    task = CustomReacher(target_size=_SMALL_TARGET, random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics, task, time_limit=time_limit, **environment_kwargs)
...
class CustomReacher(reacher.Reacher):
    """Custom multi-link reacher task."""
